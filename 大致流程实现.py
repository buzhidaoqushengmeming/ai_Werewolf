...
身份的给付与主体使用道具或行为的规则={
    ...
    '预言家':'你是预言家,你能在白天投票,如果你需要投票,请说[投票:被投票的人的编号],或者弃权请说[投票:-1].你可以在每晚查验一次别人的身份,当你需要查验别人身份时,请说[查验:玩家编号],预言家不能放弃查验.请注意,只有预言家能够查验玩家,请不要暴露自己查验的能力.',
...
while not end:
    ...
    for i, 玩家角色身份 in enumerate(玩家现状态列表):
        if 玩家角色身份 == '狼人':
            ...
            while not 狼人回合结束:
                ...
                消息列表[i] = 对狼人行为的回答
                print('{}号玩家'.format(i),狼人回答[think部分的末尾序数:], 对狼人行为的回答)
    ...
    for i,玩家角色身份 in enumerate(玩家现状态列表):
        if 玩家角色身份 == '濒死':
            濒死玩家编号 += [str(i)]
    print('狼人请闭眼(这表明狼人完成了他或他们的任务).女巫请睁眼,今天晚上这个人死了,你要救吗?或者毒药要使用吗?\n')
    ...
    for i, 玩家角色身份 in enumerate(玩家角色身份列表):
        if 玩家角色身份 == '女巫' and 玩家现状态列表[i] != '死亡':
            ...
            while not 女巫回合结束:
                ...
                print('{}号玩家'.format(i),女巫回答[think部分的末尾序数:], 对女巫行为的回答)
    # 预言家的回合
    print('女巫请闭眼(这表明女巫玩家已经完成了他的回合).预言家请睁眼,预言家请选择你要查验的玩家.\n')
    ...
    for i,玩家角色身份 in enumerate(玩家角色身份列表):
        if 玩家角色身份 == '预言家' and 玩家现状态列表[i] != '死亡':
            ...
            while not 预言家回合结束:
                ...
                消息列表[i] = 对预言家行为的回答
                print('{}号玩家'.format(i),投票者回答[think部分的末尾序数:], 对预言家行为的回答)
    # 死亡降临,猎人技能生效判定
    ...
    for 猎人编号,技能发动情况 in 猎人技能发动:
        if 技能发动情况:
            ...
            while not 猎人回合结束:
                ...
                消息列表[猎人编号] = 对猎人行为的回答
                print('{}号玩家'.format(猎人编号),猎人回答, 对猎人行为的回答)
            if 对猎人行为的回答 != '你选择弃权.':
                ...
                for 发动者编号,杀的玩家 in 猎人行为:
                    ...
                    公示 = []
                    玩家现状态列表,公示 = 代管时间变化(玩家现状态列表,公示)
                    print('{}号玩家身份为猎人,{}号玩家发动技能带走玩家,{}'.format(str(发动者编号),str(发动者编号),公示[0]))
    ...
    for 存活玩家id in 存活玩家列表:
        ...
        print('{}号玩家'.format(存活玩家id),response['message']['content'][think部分的末尾序数:])
#think部分的末尾序数 = 女巫回答.find('</think>') + len('</think>')
    # 预投票
    预投票行为统计 = []
    预投票票统计 = []
    print('现在进入投票阶段,除非投票阶段结束，或者投票结果产生争议后二审，这个阶段你不能向其它玩家说话.')
    ...
    for 存活玩家id in 存活玩家列表:
        ...
        # print('请选择你投票的玩家.如果你需要投票,请说[投票:被投票的人的编号],或者弃权请说[投票:-1],在大家投票结束后,法官会公示大家的投票对象,让大家再考虑是否更改投票,'.format(存活玩家id))
        投票者回合结束 = False
        while not 投票者回合结束:
            # 回复部分
            Agent的历史对话[int(存活玩家id)] = Agent的历史对话[int(存活玩家id)] + [{'role': 'user', 'content': 消息列表[int(存活玩家id)]}]
    ...
    print('(向大家展示投票结果)' + ','.join(预投票行为统计))
    消息列表 = 根据玩家身份为玩家发送消息(玩家现状态列表, 消息列表, '全体成员','(向大家展示投票结果)' + ','.join(预投票行为统计))
    print('大家可以根据其他人的投票结果选择是否更改自己的投票,如果你需要投票,请说[投票:被投票的人的编号],或者弃权请说[投票:-1].')
    ...
    def 递归投票过程(玩家现状态列表,投票范围,消息列表,投票轮数):
        ...
        # print(投票行为统计,'投票行为统计')
        print('(向大家展示大家投票的对象:' + ','.join(投票行为统计) + '.')
    ...
    if 投票结果 != []:
        ...
        for 猎人编号, 技能发动情况 in 猎人技能发动:
            if 技能发动情况:
                消息列表 = 根据玩家身份为玩家发送消息(玩家角色身份列表, 消息列表, 猎人编号,
                                                      '你被攻击了,现在处于濒死状态,这意味着你可以发动技能杀一个玩家,请使用[带走:玩家编号]选择你要杀的玩家,或者使用[弃权:-1]放弃发动技能带走玩家.\n')
                猎人回合结束 = False
                while not 猎人回合结束:
                    Agent的历史对话[猎人编号] = Agent的历史对话[猎人编号] + [
                        {'role': 'user', 'content': 消息列表[猎人编号]}]
                    消息列表[猎人编号] = ''
                    response: ChatResponse = chat(model=model_name, messages=Agent的历史对话[猎人编号])
                    Agent的历史对话[猎人编号] = Agent的历史对话[猎人编号] + [
                        {'role': 'assistant', 'content': response['message']['content']}]
                    ...
                    think部分的末尾序数 = 猎人回答.find('</think>') + len('</think>')
                    # print(预言家回答[think部分的末尾序数:])
                    猎人回合结束, 对猎人行为的回答 = 猎人行动(玩家角色身份列表, 玩家现状态列表,
                                                              猎人回答[think部分的末尾序数:])
                    消息列表[猎人编号] = 对猎人行为的回答
                    print('{}号玩家'.format(猎人编号), 猎人回答, 对猎人行为的回答)
                if 对猎人行为的回答 != '你选择弃权.':
                    ...
                    for 发动者编号, 杀的玩家 in 猎人行为:
                        ...
                        公示 = []
                        玩家现状态列表, 公示 = 代管时间变化(玩家现状态列表, 公示)
                        print('{}号玩家身份为猎人,{}号玩家发动技能带走玩家,{}'.format(str(发动者编号), str(发动者编号),
                                                                                      公示[0]))
                        消息列表 = 根据玩家身份为玩家发送消息(玩家现状态列表, 消息列表, '全体成员',
                                                              '{}号玩家身份为猎人,{}号玩家发动技能带走玩家,{}'.format(
                                                                  str(发动者编号), str(发动者编号), 公示[0]))
